...
class PruneExecutor:
    ...
    def purge_from_manifest(self, manifest_path: Path) -> Dict[str, any]:
        report = {'proposed_deletion': 0, 'failed': 0, 'space_reclaimed_proposal_bytes': 0, 'errors': []}
        ...
        for entity, files in manifest.items():
            if entity.startswith('_') or not isinstance(files, list):
                continue
            for f_entry in files:
                ...
                if source_path.exists():
                    try:
                        ...
                        report['proposed_deletion'] += 1
                        report['space_reclaimed_proposal_bytes'] += size
                    except Exception as e:
                        report['failed'] += 1

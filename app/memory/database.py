...
class DatabaseManager:
    ...
    def _init_db(self):
        with self._get_conn() as conn:
            ...
            # Chat history table: Stores conversation messages
            conn.execute("""
                CREATE TABLE IF NOT EXISTS chat_history (
                    ...
                    text TEXT,
                    timestamp TEXT,
                    metadata TEXT, -- JSON for things like scout/plan data
                    FOREIGN KEY(task_id) REFERENCES tasks(id)
                )
            """)
            conn.commit()
    ...
    def delete_task(self, task_id: str):
        """Deletes a task and all associated history/audit logs."""
        with self._get_conn() as conn:
            conn.execute("DELETE FROM audit_log WHERE task_id = ?", (task_id,))
            conn.execute("DELETE FROM chat_history WHERE task_id = ?", (task_id,))
    ...
    def save_message(self, task_id: str, role: str, text: str, meta: Dict = None):
        sql = """
            INSERT INTO chat_history (task_id, role, text, timestamp, metadata)
            VALUES (?, ?, ?, ?, ?)
        """
        timestamp = time.strftime("%Y-%m-%dT%H:%M:%SZ")
        with self._get_conn() as conn:
            conn.execute(sql, (task_id, role, text, timestamp, json.dumps(meta or {})))
            conn.commit()

    def get_chat_history(self, task_id: str) -> List[Dict]:
        with self._get_conn() as conn:
            rows = conn.execute("SELECT * FROM chat_history WHERE task_id = ? ORDER BY id ASC", (task_id,)).fetchall()
            messages = []
            for row in rows:
                d = dict(row)

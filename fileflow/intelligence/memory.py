...
Embeddings: nomic-embed-text via Ollama (bridge.py)
"""
...
SCHEMA_FIELDS = [
    ...
    ("entity", pa.string()),
    ("summary", pa.string()),
    ("embedding", pa.list_(pa.float32(), 768)),  # nomic-embed-text dim
...
class Memory:
    ...
    def remember(
        ...
        # Pad or truncate embedding to exactly 768 dims (nomic-embed-text)
        embedding = self._normalise_embedding(embedding, 768)
    ...
    def _already_indexed(self, file_path: str, content_hash: str) -> bool:
        ...
        try:
            results = (
                self._table.search()
                .where(f"file_path = '{file_path}' AND content_hash = '{content_hash}'")
                .limit(1)
                .to_list()
            )
